const UVAFunctions = () =>{addObserverEmailButton(); libraryMainNav(); sortGradesObservers(); iframeLonger(); logIn(); addChooseChild(); replaceSupportButton(); saraSudo(); replacePointsWithPercentage(); studentView(); moduleGB();}
...
function iframeLonger() {
  ...
  checkIfNull("#main").then(() => {
    ...
    const edgyCourses = ["1756", "2737", "1757", "2738", "1758", "41", "1035"];
...
function missingSubmissionsObserver() {
    ...
    checkIfNull(".user-observees").then(() => {
        if (ENV.current_user_roles.includes("observer")) {
            const delay = ms => new Promise(res => setTimeout(res, ms));
            const observeeContainer = document.querySelector(
              ".observees-list-container"
            );
            const observeeList = Array.prototype.slice.apply(document.querySelectorAll(".observees-list li"));
            ...
            fetchObserveesID().then(() => {
              found = observees.find( val => observeeList.name.includes(val.innerHTML) )
              console.log(found);
              const students = 
                (msgHolder.innerHTML = '<h2> ${observees.childName}</h2>')
              students;
            });
          }
        });
